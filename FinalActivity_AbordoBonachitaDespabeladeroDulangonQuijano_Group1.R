...
create_styles <- function() {
  tags$style(HTML("
    ...
    /* MathJax safety net -- defend against the v2 quirk where the
       pre-render preview span occasionally stays visible alongside the
       rendered math, producing overlapping/garbled text. */
    .MathJax_Preview { display: none !important; }
    .MathJax { outline: none; }
    /* Inline math inside <summary> headings (used in the How-it-works
       steps) needs a stable line-height so the rendered span doesn't
       bleed into the trailing literal text. */
    details.step-box > summary h4 .MathJax,
    details.step-box > summary h4 .MathJax_Display { line-height: 1; }
  "))
}
